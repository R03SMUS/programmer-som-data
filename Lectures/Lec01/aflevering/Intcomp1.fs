...
(*2.1*)
    | Let([], ebody) ->
      eval ebody env
    | Let((string,expr)::tail, ebody) -> 
      let xval = eval expr env
      let env1 = (string, xval) :: env 
      eval (Let(tail, ebody)) env1
(*2.1*)
...
let e1 = Let([("x", CstI 10); ("y", Prim("+", Var "x", CstI 1))], Prim("+", Var "x", Var "y"))
...
let rec freevars e : string list =
    ...
    | Var x  -> [x]
    | Let((string, expr)::tail, ebody) -> 
            union (freevars expr, minus (freevars ebody, [string]))
    | Let([], ebody) -> 
        freevars ebody
...
let rec tcomp (e : expr) (cenv : string list) : texpr =
    ...
    | Let((string, expr)::tail, ebody) -> (*2.3*)
      let cenv1 = string :: cenv 
      tcomp (Let(tail, ebody)) cenv1
    | Let([], ebody) -> 
      tcomp ebody cenv
